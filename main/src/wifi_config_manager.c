...
{
    if (s_scan_state == WIFI_SCAN_STATE_SCANNING) {
        return;
    }
    s_scan_state = WIFI_SCAN_STATE_SCANNING;
    s_scan_count = 0;
...
bool wifi_config_manager_load(char *ssid_out, size_t ssid_len, char *pass_out, size_t pass_len)
{
    ...
    if (!prv_load_wifi_list(&list) || list.count == 0) {
        return false;
    }
...
{
    if (host == NULL || dev == NULL || sym == NULL) {
        return false;
    }
    ...
    if (err == ESP_OK) {
        err = nvs_set_str(my_handle, "az_dev", dev);
    }
    if (err == ESP_OK) {
        err = nvs_set_str(my_handle, "az_sym", sym);
    }
