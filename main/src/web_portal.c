...
static const char *portal_html =
    ...
    "        </div>"
    "        <div style='margin-top:8px;display:flex;justify-content:center;gap:8px;max-width:380px;margin-left:auto;margin-right:auto;'>"
    ...
    "          <button class='btn-secondary btn-ctrl' onclick='simulateBtn(\"right\")' style='flex:1;padding:10px 0;margin:0;'>► RIGHT</button>"
...
"}"
    ...
    "if(!document.getElementById('tab-lcd').classList.contains('active'))return;"
    "if(['INPUT','TEXTAREA','SELECT'].includes(document.activeElement.tagName))return;"
    "if(e.key==='ArrowRight'){simulateBtn('right');e.preventDefault();}"
    "else if(e.key==='ArrowUp'){simulateBtn('up');e.preventDefault();}"
    "else if(e.key==='ArrowDown'){simulateBtn('down');e.preventDefault();}"
